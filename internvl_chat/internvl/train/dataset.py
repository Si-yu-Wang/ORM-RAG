...
def rag_preprocess_mpt(
    ...
    assert len(num_image_token_list) >= num_ref_image + num_image, "num_image_token_list长度不足，需包含参考图和输入图的token数"

    conversations = []
    for i, source in enumerate(sources):
        first_human_idx = next((j for j, s in enumerate(source) if s['from'] == 'human'), None)
        ...
        if first_human_idx is not None:
            ...
            source[first_human_idx]['value'] = rag_prompt + source[first_human_idx]['value']
            if 'Please help me generate a report for this video' in source[first_human_idx]['value']:
                source[first_human_idx]['value'] = source[first_human_idx]['value'].replace('Please help me generate a report for this video','Please generate a fetal ultrasound examination report according to the similar images and reports retrieved above and the multiple images I gave. The retrieval information may contain the same abnormal parts as the multiple images I gave, or it may be different. Please make a specific judgment according to the multiple images I gave.')
        ...
        for j, sentence in enumerate(source):
            role = roles[sentence['from']]
            assert role == conv.roles[j % 2], f'对话{i}的角色交替错误'
            conv.append_message(role, sentence['value'])
        conversations.append(conv.get_prompt())
    ...
    if not text_only:
        new_conversations = []
        for conversation in conversations:
            total_replace = num_ref_image + num_image
            for i in range(total_replace):
                image_tokens = f'{IMG_START_TOKEN}{IMG_CONTEXT_TOKEN * num_image_token_list[i]}{IMG_END_TOKEN}'
                conversation = conversation.replace('<image>', image_tokens, 1)
            new_conversations.append(conversation)
        conversations = new_conversations
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for idx in range(3, len(turns), 2):
            re_turns.append(conv.sep.join(turns[idx:idx+2]))
        ...
        for turn in re_turns:
            ...
            instruction_len = len(tokenizer(parts[0] + sep).input_ids)
        ...
        if cur_len < tokenizer.model_max_length and cur_len != total_len:
            print(f"WARNING: 长度不匹配 {cur_len} vs {total_len}，数据集 {ds_name}")
            target[:] = IGNORE_TOKEN_ID
