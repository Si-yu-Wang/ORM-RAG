...
from internvl.train.dataset import (ConcatDataset, TCSLoader,
                                    ...
                                    preprocess_internlm, preprocess_mpt, rag_preprocess_mpt,
                                    preprocess_phi3)
...
class DataTrainingArguments:
    ...
    rag_usage: bool = field(
        default=False,
        metadata={'help': 'Set to True to enable the usage of RAG.'}
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def get_preprocess_function(self):
        ...
        if self.template_name == 'Hermes-2':
            if self.rag_usage:
                preprocess_function = rag_preprocess_mpt
            else:
    ...
    def multi_modal_multi_image_get_item(self, data_item):
        ...
        if self.rag_usage:
            if 'retrieval_reports' in data_item:
            # 得到relative_image和relative_report
                relative_report=[]
                relative_image=[]
                relative_report.append(data_item['retrieval_reports'])
                relative_image.append(data_item['retrieval_images'])
                # relative_report=data_item['retrieval_reports']
                # relative_image=data_item['retrieval_images']
            else:
                relative_report=[]
                relative_image=[]
            relative_images_num = len(relative_image)
            image_list = relative_image + image_list
        ...
        if self.rag_usage:
            ret = preprocess_function(self.template_name, [deepcopy(data_item['conversations'])],
                                    ...
                                    ds_name=self.ds_name, num_image=num_image, reference_reports=relative_report, num_ref_image=relative_images_num)
        else:
            ret = preprocess_function(self.template_name, [deepcopy(data_item['conversations'])],
                                    self.tokenizer, num_image_tokens, group_by_length=self.group_by_length,
...
def main():
    ...
    train_dataset = build_datasets(
        ...
        min_dynamic_patch=data_args.min_dynamic_patch, max_dynamic_patch=data_args.max_dynamic_patch,
        normalize_type=data_args.normalize_type, rag_usage=data_args.rag_usage)
